...
class AI(context: Context) {
    ...
    fun getAnswer(question:String, callback: Consumer<String?>){
        ...
        var matchDiffBetweenDatesQuestion = diffBetweenDatesReg.find(question)
        ...
        while (matchWeatherQuestion != null){
            ...
            val cityName = matchWeatherQuestion.value
                .drop(strFirstPath!!.value.length)
                .replace("?", "")
            GeocodeLocale().getGeocode(cityName) { point ->
                ForecastToString().getForecast(point, cityName, currentLanguage,
