...
class AI(context: Context) {
    ...
    fun getAnswer(question:String, callback: Consumer<String?>){
        ...
        while (matchHolidayQuestion != null) {
            answer += getHolidaysOnDate(matchHolidayQuestion.value, callback)
        ...
        while (matchWeatherQuestion != null){
            getCurrentWeather(matchWeatherQuestion.value, callback)
    ...
    private fun getCurrentWeather(question: String, callback: Consumer<String?>){
        val strFirstPath = weatherQuestionFirstPath.find(question)
        var cityName = question
    ...
    private fun getHolidaysOnDate(question: String, callback: Consumer<String?>): String{
        try {
            ...
            val result = ParsingHtmlService().getHoliday(
                date, wrongDateOfHolidayAnswer, defaultHolidayAnswer, correctHolidayAnswer
            )
            callback.accept("$result.")
            return "$result. "
        }catch (e: Exception){
            callback.accept(e.message)
            return "${e.message} "
        }
    }
