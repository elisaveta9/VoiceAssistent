...
class AI(context: Context) {
    ...
    private val queAndAns = mapOf(
        ...
        context.getString(R.string.date_today_pattern).toRegex() to LocalDateTime.now()
            .format(DateTimeFormatter.ofPattern("dd MMM yyyy")).toString(),
        context.getString(R.string.what_time_is_it_pattern).toRegex() to LocalDateTime.now()
        ...
        context.getString(R.string.day_of_the_week_today_pattern).toRegex() to
                daysOfWeek.get(LocalDate.now().dayOfWeek)
    )
